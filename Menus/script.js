...
document.addEventListener('DOMContentLoaded', () => {
    renderNavigation(); // Gera o menu dinamicamente
    initMobileMenu();   // Ativa o menu hambúrguer no mobile
});

// --- CONFIGURAÇÃO DO MENU (Mude aqui para atualizar todo o site) ---
const menuConfig = [
        ...
        subcategories: [
            {
                ...
                links: [
					...
					{ label: 'Cadastros Centralizados de Contribuintes', href: 'https://dfe-portal.svrs.rs.gov.br/NFE/CCC' }
                ]
            },
            {
...
function renderNavigation() {
    ...
    menuConfig.forEach(item => {
        if (!item.isDropdown) {
            ...
            // Dropdown que abre categorias para a esquerda
            html += `
                        ...
                        ${item.subcategories.map(sub => `
                            <div class="dropdown-sub-container">
                                ...
                                <div class="dropdown-submenu">
                                    ${sub.links.map(link => `
                                        <a href="${link.href}" target="_blank" class="dropdown-item">${link.label}</a>
                                    `).join('')}
                                </div>
                            </div>
                        `).join('')}
