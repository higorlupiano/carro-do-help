...
function createVisualLabel(top, left, height, width) {
    ...
    if (document.getElementById('showBarcode').checked) {
        ...
        const userCode = (document.getElementById('productCode').value || '123456789012').trim();
        const barcodeUrl = getBarcodeDataUrl(userCode);
    ...
    // --- TEXTOS (lidos dos campos do usuário) ---
    const userCode = (document.getElementById('productCode').value || '').trim();
    const userName = (document.getElementById('productName').value || '').trim();
    const userPriceRaw = (document.getElementById('productPrice').value || '').trim();
    const userPrice = userPriceRaw ? 'R$ ' + userPriceRaw : 'R$ 0,00';
    const userCustom = (document.getElementById('productCustom').value || '').trim();

    addVisualText(el, 'showCode', 'codeTop', 'codeSize', userCode || '123456789012');
    addVisualText(el, 'showDesc', 'descTop', 'descSize', userName || 'PRODUTO TESTE');
    addVisualText(el, 'showPrice', 'priceTop', 'priceSize', userPrice);
    addVisualText(el, 'showCustom', 'customTop', 'customSize', userCustom);
...
async function generatePDF() {
    try {
        ...
        // Prepara barcode usando o código do usuário
        const userCode = (document.getElementById('productCode').value || '123456789012').trim();
        const userName = (document.getElementById('productName').value || 'PRODUTO TESTE').trim();
        const userPriceRaw = (document.getElementById('productPrice').value || '0,00').trim();
        const userPrice = 'R$ ' + userPriceRaw;
        const userCustom = (document.getElementById('productCustom').value || '').trim();
        const barcodeUrl = getBarcodeDataUrl(userCode);
        ...
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                ...
                addPdfText('showCode', 'codeTop', 'codeSize', userCode);
                addPdfText('showDesc', 'descTop', 'descSize', userName);
                addPdfText('showPrice', 'priceTop', 'priceSize', userPrice);
                if (userCustom) addPdfText('showCustom', 'customTop', 'customSize', userCustom);
            }
        }
